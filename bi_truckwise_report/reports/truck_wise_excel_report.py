...
class TruckWiseExcelReport(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, sale):
        ...
        month_to = data["form"]["month_to"]
        year = int(data["form"]["year"])

        from_date_month_num = datetime.strptime(month_from, '%b').month
        to_date_month_num = datetime.strptime(month_to, '%b').month
        ...
        truck_analytic_records = self.env['account.analytic.account'].search([])
        truck_analytic_sorted_records = sorted(truck_analytic_records)
        truck_row_no=3
        truck_col_no=0
        for rec in truck_analytic_sorted_records:
            sheet.write(truck_row_no,truck_col_no,rec.name,format_1 )
            truck_row_no = truck_row_no+1
        ...
        for mnth in months_list:
            ...
            sale_ids = self.env['sale.order'].search([('date_order','>=',month_start_date),('date_order','<=',month_last_date),('state','=',"job_completed")])
            if sale_ids:
                query = """
                ...
                sheet.write(truck_row, truck_col,"",format_1 )
                truck_col+=1
                sheet.write(truck_row, truck_col,'',format_1 )
                truck_col+=1
                sheet.write(truck_row, truck_col,'',format_1 )
                ...
                sheet.write(truck_row, truck_col,"",format_1 )
                truck_row += 1 
                truck_col = truck_col- 6
            # else:
            #     sheet.write(truck_row, truck_col,0,format_1 )
            #     truck_col+=1
            #     sheet.write(truck_row, truck_col,0,format_1 )
            #     truck_col+=1
            #     sheet.write(truck_row, truck_col,0,format_1 )
